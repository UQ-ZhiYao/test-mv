function togglePortfolioValue(){portfolioVisible=!portfolioVisible;var icon=document.getElementById("eyeIcon");if(icon)icon.innerHTML=portfolioVisible?'<path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/>':'<path d="M17.94 17.94A10.07 10.07 0 0112 20c-7 0-11-8-11-8a18.45 18.45 0 015.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0112 4c7 0 11 8 11 8a18.5 18.5 0 01-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/>';applyEyeVisibility()}function drawSparkline(){var c=document.getElementById("sparkChart");if(!c)return;var w=100,h=44;c.width=w;c.height=h;var ctx=c.getContext("2d");var pts=[56224,58100,59800,61200,60400,59100,57800,58700,60200,59600,60800,61228];var min=Math.min.apply(null,pts),max=Math.max.apply(null,pts);var pX=2,pY=6;function sx(i){return pX+i/(pts.length-1)*(w-pX*2)}function sy(v){return h-pY-(v-min)/(max-min)*(h-pY*2)}var avgY=sy(pts.reduce(function(a,b){return a+b},0)/pts.length);ctx.save();ctx.setLineDash([3,3]);ctx.beginPath();ctx.moveTo(pX,avgY);ctx.lineTo(w-pX,avgY);ctx.strokeStyle="rgba(0,0,0,.2)";ctx.lineWidth=1;ctx.stroke();ctx.restore();var grad=ctx.createLinearGradient(0,0,0,h);grad.addColorStop(0,"rgba(21,101,192,0.25)");grad.addColorStop(1,"rgba(21,101,192,0.02)");ctx.beginPath();ctx.moveTo(sx(0),sy(pts[0]));for(var i=1;i<pts.length;i++)ctx.lineTo(sx(i),sy(pts[i]));ctx.lineTo(sx(pts.length-1),h);ctx.lineTo(sx(0),h);ctx.closePath();ctx.fillStyle=grad;ctx.fill();ctx.beginPath();ctx.moveTo(sx(0),sy(pts[0]));for(var i=1;i<pts.length;i++)ctx.lineTo(sx(i),sy(pts[i]));ctx.strokeStyle="#1565C0";ctx.lineWidth=2;ctx.lineJoin="round";ctx.lineCap="round";ctx.stroke();ctx.beginPath();ctx.arc(sx(pts.length-1),sy(pts[pts.length-1]),3,0,Math.PI*2);ctx.fillStyle="#1565C0";ctx.fill()}var TX_DATA=[];function fmtTxDate(d){var dt=new Date(d+"T00:00:00");return dt.toLocaleDateString("en-MY",{day:"2-digit",month:"short",year:"numeric"})}function buildTxDataFromCi(){var rows=[];function addRows(ciRows,acct){(ciRows||[]).forEach(function(r){var u=parseFloat(r.units)||0,a=parseFloat(r.amount)||0;var type=r.type||(u>=0?"Subscription":"Redemption");rows.push({date:fmtTxDate(r.date),_sortDate:r.date,ref:r.reference_id||"—",acct:acct,kind:type==="Redemption"?"withdrawal":"deposit",approved:r.status==="Approved",amt:Math.abs(a),units:Math.abs(u),txId:r.id||null})})}addRows(PA_CI_ROWS,"pa");addRows(JA_CI_ROWS,"ja");rows.sort(function(a,b){return a._sortDate<b._sortDate?1:a._sortDate>b._sortDate?-1:0});return rows}
function stmtForTx(txId){if(!txId)return null;return(STATEMENTS||[]).filter(function(s){return s.transaction_id===txId})[0]||null}
function stmtForInvestorDistribution(investorId,distributionId){if(!investorId||!distributionId)return null;return(STATEMENTS||[]).filter(function(s){return s.type==="Dividend"&&s.investor_id===investorId&&s.distribution_id===distributionId})[0]||null}
// Mobile Safari's native PDF plugin inside an <iframe> is unreliable for
// multi-page documents — it can lock the iframe's rendered height to
// page 1's intrinsic size and never let the rest scroll into view. Render
// each page ourselves with pdf.js into plain <canvas> elements stacked in
// an ordinary scrolling <div> instead — that sidesteps the plugin
// entirely, so scrolling is just normal DOM scrolling.
var ZY_PDFJS_READY=null;
function zyLoadPdfJs(){if(ZY_PDFJS_READY)return ZY_PDFJS_READY;ZY_PDFJS_READY=import("https://cdn.jsdelivr.net/npm/pdfjs-dist@4.7.76/build/pdf.min.mjs").then(function(lib){lib.GlobalWorkerOptions.workerSrc="https://cdn.jsdelivr.net/npm/pdfjs-dist@4.7.76/build/pdf.worker.min.mjs";return lib});return ZY_PDFJS_READY}
// Zoom deliberately does NOT use the browser's native pinch-zoom (page
// viewport scale) — that would zoom #stmtViewerOverlay's fixed header
// bar right along with the content. Instead a 2-finger pinch just resizes
// #stmtViewerZoomLayer (the canvases' real parent) wider/narrower; each
// canvas is width:100% of that layer, so it visually scales with it, and
// #stmtViewerFrame's native overflow:auto scrolling pans around it in
// both directions once it's bigger than the frame — nothing outside
// #stmtViewerFrame ever moves or scales.
var ZY_STMT_ZOOM=1, ZY_STMT_PINCH=null, ZY_STMT_ZOOM_WIRED=false;
function zyTouchDist(touches){var dx=touches[0].clientX-touches[1].clientX,dy=touches[0].clientY-touches[1].clientY;return Math.sqrt(dx*dx+dy*dy)}
function zyStmtZoomInit(){
  if(ZY_STMT_ZOOM_WIRED)return;
  var frame=document.getElementById("stmtViewerFrame");
  if(!frame)return;
  ZY_STMT_ZOOM_WIRED=true;
  frame.addEventListener("touchstart",function(e){
    if(e.touches.length===2) ZY_STMT_PINCH={dist:zyTouchDist(e.touches),zoom:ZY_STMT_ZOOM};
  },{passive:true});
  frame.addEventListener("touchmove",function(e){
    if(e.touches.length!==2||!ZY_STMT_PINCH) return;
    e.preventDefault();
    var scale=ZY_STMT_PINCH.zoom*(zyTouchDist(e.touches)/ZY_STMT_PINCH.dist);
    scale=Math.max(1,Math.min(scale,4));
    ZY_STMT_ZOOM=scale;
    var layer=document.getElementById("stmtViewerZoomLayer");
    if(layer) layer.style.width=(scale*100)+"%";
  },{passive:false});
  frame.addEventListener("touchend",function(e){
    if(e.touches.length<2) ZY_STMT_PINCH=null;
  },{passive:true});
}
async function openZyStatement(path,fileName){try{if(typeof sb==="undefined"||!sb||!path)return;var res=await sb.storage.from("statements").createSignedUrl(path,300);if(res.error){if(typeof showToastM==="function")showToastM(typeof t==="function"?t("statement.openFailed"):"Could not open statement");return}var overlay=document.getElementById("stmtViewerOverlay"),holder=document.getElementById("stmtViewerFrame"),nameEl=document.getElementById("stmtViewerName");if(!overlay||!holder){window.open(res.data.signedUrl,"_blank");return}if(nameEl)nameEl.textContent=fileName||(typeof t==="function"?t("statement.viewerTitle"):"Statement");holder.innerHTML='<div style="padding:60px 0;text-align:center;color:var(--fg-3);font-size:.85rem;">Loading…</div>';overlay.style.display="flex";zyStmtZoomInit();ZY_STMT_ZOOM=1;ZY_STMT_PINCH=null;var lib=await zyLoadPdfJs();var pdfDoc=await lib.getDocument(res.data.signedUrl).promise;holder.innerHTML="";var layer=document.createElement("div");layer.id="stmtViewerZoomLayer";layer.style.cssText="width:100%;";holder.appendChild(layer);var dpr=Math.min(window.devicePixelRatio||1,3);var pageW=Math.max((holder.clientWidth||320)-16,240);for(var i=1;i<=pdfDoc.numPages;i++){var page=await pdfDoc.getPage(i);var scale=pageW/page.getViewport({scale:1}).width;var viewport=page.getViewport({scale:scale*dpr});var canvas=document.createElement("canvas");canvas.width=viewport.width;canvas.height=viewport.height;canvas.style.cssText="display:block;width:100%;height:auto;margin:0 0 10px;border-radius:4px;box-shadow:0 1px 6px rgba(0,0,0,.15);background:#fff;";layer.appendChild(canvas);await page.render({canvasContext:canvas.getContext("2d"),viewport:viewport}).promise}}catch(e){console.warn("Open statement failed:",e.message);var holder2=document.getElementById("stmtViewerFrame");if(holder2)holder2.innerHTML='<div style="padding:60px 24px;text-align:center;color:var(--fg-3);font-size:.85rem;">Could not open statement</div>'}}
function closeZyStatementViewer(){var overlay=document.getElementById("stmtViewerOverlay"),holder=document.getElementById("stmtViewerFrame");if(overlay)overlay.style.display="none";if(holder){holder.innerHTML="";holder.scrollLeft=0;holder.scrollTop=0}ZY_STMT_ZOOM=1;ZY_STMT_PINCH=null}var txFilter="all";function setTxFilter(f){txFilter=f;["all","deposit","withdrawal"].forEach(function(k){var b=document.getElementById("txf-"+k);if(b)b.classList.toggle("on",k===f)});renderTxList()}function fmtTxRM(n){return n.toLocaleString("en-MY",{minimumFractionDigits:2,maximumFractionDigits:2})}function renderTxList(){var sel=document.getElementById("txAcctSelect");if(!sel)return;var acct=sel.value;var rows=TX_DATA.filter(function(t){return(txFilter==="all"||t.kind===txFilter)&&(acct==="all"||t.acct===acct)});var list=document.getElementById("txList");if(!list)return;var ttTxL=typeof t==="function"?t:function(k){return k};if(!rows.length){list.innerHTML='<div class="tx-empty">'+ttTxL("transaction.noTransactionsFound")+"</div>";return}list.innerHTML=rows.map(function(t){var stmt=stmtForTx(t.txId);var rowCls=stmt?"tx-row tx-row-link":"tx-row";var rowClick=stmt?' onclick="openZyStatement(\''+stmt.storage_path+"','"+(stmt.file_name||"")+'\')"':"";return'<div class="'+rowCls+'"'+rowClick+'>'+'<div class="txr-left"><div class="txr-date">'+t.date+'</div><div class="txr-type">'+t.ref+"</div></div>"+'<div class="txr-right"><div class="txr-amt '+(t.kind==="deposit"?"dep":"wd")+'">'+(t.kind==="deposit"?"+":"-")+"RM "+fmtTxRM(t.amt)+'</div><div class="tx-units">'+(t.approved?fmtTxRM(t.units)+ttTxL("sheet.unitsSuffix"):ttTxL("transaction.pending"))+"</div></div>"+"</div>"}).join("")}
var STMT_DATA=[];
function buildStmtDataFromStatements(){var rows=[];function addRows(acct,investorId){if(!investorId)return;(STATEMENTS||[]).filter(function(s){return s.type==="Annual"&&s.investor_id===investorId}).forEach(function(s){var d=FY_END_DATE_BY_ID[s.fy_id]||(s.generated_at||"").slice(0,10);rows.push({date:d?fmtTxDate(d):"—",_sortDate:d,acct:acct,label:s.period_label||"—",storage_path:s.storage_path,file_name:s.file_name})})}addRows("pa",PROFILE&&PROFILE.id);addRows("ja",PROFILE&&PROFILE.joint_account_id);rows.sort(function(a,b){return a._sortDate<b._sortDate?1:a._sortDate>b._sortDate?-1:0});return rows}
function renderStmtList(){var sel=document.getElementById("stmtAcctSelect");if(!sel)return;var acct=sel.value;var rows=STMT_DATA.filter(function(s){return acct==="all"||s.acct===acct});var list=document.getElementById("stmtList");if(!list)return;var ttStL=typeof t==="function"?t:function(k){return k};if(!rows.length){list.innerHTML='<div class="tx-empty">'+ttStL("statement.noStatementsFound")+"</div>";return}list.innerHTML=rows.map(function(s){return'<div class="tx-row tx-row-link" onclick="openZyStatement(\''+s.storage_path+"','"+(s.file_name||"")+'\')">'+'<div class="txr-left"><div class="txr-date">'+s.date+'</div><div class="txr-type">'+ttStL("statement.investmentAccountStatement")+"</div></div>"+'<div class="txr-right"><div class="txr-amt">'+s.label+"</div></div>"+"</div>"}).join("")}
function renderStatementTab(){var rows=(STMT_DATA||[]).filter(function(s){return s.acct===AD_ACCT});var list=document.getElementById("adStmtList");if(!list)return;var ttAdSt=typeof t==="function"?t:function(k){return k};if(!rows.length){list.innerHTML='<div class="tx-empty">'+ttAdSt("statement.noStatementsFound")+"</div>";return}list.innerHTML=rows.map(function(s){return'<div class="tx-row tx-row-link" onclick="openZyStatement(\''+s.storage_path+"','"+(s.file_name||"")+'\')">'+'<div class="txr-left"><div class="txr-date">'+s.date+'</div><div class="txr-type">'+ttAdSt("statement.investmentAccountStatement")+"</div></div>"+'<div class="txr-right"><div class="txr-amt">'+s.label+"</div></div>"+"</div>"}).join("")}
var DX_DATA=[];async function loadDistributionHistory(){try{if(typeof sb==="undefined"||!sb)return;var res=await sb.from("distributions").select("id, fy, dps, ex_date, pay_date, status").eq("status","Paid").order("pay_date",{ascending:false});if(res.error){console.warn("[Distributions] load failed:",res.error.message);return}var distRows=res.data||[];var rows=[];distRows.forEach(function(d){[["pa",PA_CI_ROWS,PROFILE&&PROFILE.id],["ja",JA_CI_ROWS,PROFILE&&PROFILE.joint_account_id]].forEach(function(pair){var acct=pair[0],ciRows=pair[1],investorId=pair[2];if(!ciRows||!ciRows.length)return;var unitsAtExDate=0;ciRows.forEach(function(r){if(r.date<=d.ex_date)unitsAtExDate+=parseFloat(r.units)||0});if(unitsAtExDate<=1e-4)return;var amt=unitsAtExDate*(parseFloat(d.dps)||0)/100;var payDate=d.pay_date||d.ex_date;rows.push({date:fmtTxDate(payDate),_sortDate:payDate,type:"distribution.cashPayout",acct:acct,kind:"cash",amt:amt,units:0,investorId:investorId||null,distributionId:d.id})})});rows.sort(function(a,b){return a._sortDate<b._sortDate?1:a._sortDate>b._sortDate?-1:0});DX_DATA=rows;if(document.getElementById("dxList"))renderDxList()}catch(e){console.warn("[Distributions] load failed:",e.message)}}var dxFilter="all";function setDxFilter(f){dxFilter=f;["all","cash","reinvest"].forEach(function(k){var b=document.getElementById("dxf-"+k);if(b)b.classList.toggle("on",k===f)});renderDxList()}function fmtDxRM(n){return n.toLocaleString("en-MY",{minimumFractionDigits:2,maximumFractionDigits:2})}function renderDxList(){var sel=document.getElementById("dxAcctSelect");if(!sel)return;var acct=sel.value;var rows=DX_DATA.filter(function(t){return(dxFilter==="all"||t.kind===dxFilter)&&(acct==="all"||t.acct===acct)});var list=document.getElementById("dxList");if(!list)return;var ttDxL=typeof t==="function"?t:function(k){return k};if(!rows.length){list.innerHTML='<div class="dx-empty">'+ttDxL("distribution.noDistributionsFound")+"</div>";return}list.innerHTML=rows.map(function(t){var stmt=stmtForInvestorDistribution(t.investorId,t.distributionId);var rowCls=stmt?"dx-row dx-row-link":"dx-row";var rowClick=stmt?' onclick="openZyStatement(\''+stmt.storage_path+"','"+(stmt.file_name||"")+'\')"':"";return'<div class="'+rowCls+'"'+rowClick+'>'+'<div class="dxr-left"><div class="dxr-date">'+t.date+'</div><div class="dxr-type">'+(stmt&&stmt.reference_id?stmt.reference_id:"—")+"</div></div>"+'<div class="dxr-right"><div class="dxr-amt">+RM '+fmtDxRM(t.amt)+'</div><div class="dx-units">'+(t.units>0?fmtDxRM(t.units)+ttDxL("sheet.unitsSuffix"):"—")+"</div></div>"+"</div>"}).join("")}function renderPrincipalTab(){var rows=(TX_DATA||[]).filter(function(t){return t.acct===AD_ACCT});var list=document.getElementById("adTxList");if(!list)return;var ttAdTx=typeof t==="function"?t:function(k){return k};if(!rows.length){list.innerHTML='<div class="tx-empty">'+ttAdTx("transaction.noTransactionsFound")+"</div>";return}list.innerHTML=rows.map(function(t){var stmt=stmtForTx(t.txId);var rowCls=stmt?"tx-row tx-row-link":"tx-row";var rowClick=stmt?' onclick="openZyStatement(\''+stmt.storage_path+"','"+(stmt.file_name||"")+'\')"':"";return'<div class="'+rowCls+'"'+rowClick+'>'+'<div class="txr-left"><div class="txr-date">'+t.date+'</div><div class="txr-type">'+t.ref+"</div></div>"+'<div class="txr-right"><div class="txr-amt '+(t.kind==="deposit"?"dep":"wd")+'">'+(t.kind==="deposit"?"+":"-")+"RM "+fmtTxRM(t.amt)+'</div><div class="tx-units">'+(t.approved?fmtTxRM(t.units)+ttAdTx("sheet.unitsSuffix"):ttAdTx("transaction.pending"))+"</div></div>"+"</div>"}).join("")}function renderDistributionTab(){var rows=(DX_DATA||[]).filter(function(t){return t.acct===AD_ACCT});var list=document.getElementById("adDxList");if(!list)return;var ttAdDx=typeof t==="function"?t:function(k){return k};if(!rows.length){list.innerHTML='<div class="dx-empty">'+ttAdDx("distribution.noDistributionsFound")+"</div>";return}list.innerHTML=rows.map(function(t){var stmt=stmtForInvestorDistribution(t.investorId,t.distributionId);var rowCls=stmt?"dx-row dx-row-link":"dx-row";var rowClick=stmt?' onclick="openZyStatement(\''+stmt.storage_path+"','"+(stmt.file_name||"")+'\')"':"";return'<div class="'+rowCls+'"'+rowClick+'>'+'<div class="dxr-left"><div class="dxr-date">'+t.date+'</div><div class="dxr-type">'+(stmt&&stmt.reference_id?stmt.reference_id:"—")+"</div></div>"+'<div class="dxr-right"><div class="dxr-amt">+RM '+fmtDxRM(t.amt)+'</div><div class="dx-units">'+(t.units>0?fmtDxRM(t.units)+ttAdDx("sheet.unitsSuffix"):"—")+"</div></div>"+"</div>"}).join("")}function drawAdDonut(retriesLeft){var c=document.getElementById("adDonut");if(!c)return;var paMv=PA_ACCT?PA_ACCT.mv:0,jaMv=JA_ACCT?JA_ACCT.mv:0;var total=paMv+jaMv;var paPct=total?paMv/total*100:0,jaPct=total?jaMv/total*100:0;var totalEl=document.getElementById("adDonutTotal");if(totalEl){totalEl.setAttribute("data-real",fmtMoney(total));totalEl.textContent=portfolioVisible?fmtMoney(total):"••••••"}var pEl=document.getElementById("adLegendPersonal");if(pEl)pEl.textContent=paPct.toFixed(1)+"%";var jEl=document.getElementById("adLegendJoint");if(jEl)jEl.textContent=jaPct.toFixed(1)+"%";var jWrap=document.getElementById("adLegendJointWrap");if(jWrap)jWrap.style.display=JA_ACCT?"":"none";var dpr=window.devicePixelRatio||1;var W=c.parentElement.parentElement.clientWidth-32;var size=Math.min(W,260);if(size<=0){if(retriesLeft===undefined)retriesLeft=10;if(retriesLeft>0)setTimeout(function(){drawAdDonut(retriesLeft-1)},100);return}var H=size*.58;c.width=size*dpr;c.height=H*dpr;c.style.width=size+"px";c.style.height=H+"px";var ctx=c.getContext("2d");ctx.scale(dpr,dpr);var cx=size/2,cy=H*.96,r=size*.42,lw=r*.11;var segs=JA_ACCT?[{v:paPct,c:"#64B5F6"},{v:jaPct,c:"#B39DDB"}]:[{v:paPct||100,c:"#64B5F6"}];var start=Math.PI;ctx.lineCap="butt";segs.forEach(function(s){var ang=s.v/100*Math.PI;ctx.beginPath();ctx.arc(cx,cy,r,start,start+ang);ctx.lineWidth=lw;ctx.strokeStyle=s.c;ctx.stroke();start+=ang});var txt=document.getElementById("adDonutText");if(txt){txt.style.top=cy-6+"px";txt.style.transform="translateY(-100%)"}}var trendPeriod="ytd";var trendCustomFrom=null,trendCustomTo=null;var adTrendState={};
...
async function loadAdTrendData(period){
  ...
  else if(period==="custom"&&trendCustomFrom){cutoff=new Date(trendCustomFrom+"T00:00:00")}
  ...
  var q=sb.from("nta_daily").select("date,nta").order("date",{ascending:true}).gte("date",effectiveCutoff).limit(5000);
  if(period==="custom"&&trendCustomTo)q=q.lte("date",trendCustomTo);
  var res=await q;
  var ntaRows=res.data||[];
  var idx=0,cumUnits=0,cumDeposit=0,vals=[],deps=[],lbls=[];
  ntaRows.forEach(function(r){
    while(idx<allCi.length&&allCi[idx].date<=r.date){
    ...
    deps.push(cumDeposit);
    lbls.push(r.date)
  });
  return{v:vals,d:deps,l:lbls}
}
...
function fmtPctAxisLabel(v){return v.toFixed(1)+"%"}
...
async function drawAdTrend(period){
  ...
  attachAdTrendListeners();
  drawAdTrendFrame(null)
}
...
function drawAdTrendFrame(hoverIdx){
  ...
  var vals=s.data.v,deps=s.data.d;
...
// Trends) and derives, per point: P/L = asset value - net deposit
// (still shown in the hover readout), and Return % = P/L / net deposit
// (what the chart itself now plots — see drawPlTrendFrame). Return is
// NaN wherever net deposit isn't positive (e.g. a point at/before the
// very first deposit), so it can be safely dropped from the axis
// min/max and from any line segment via canvas's built-in NaN handling.
async function loadPlTrendData(period){
  ...
  var ret=data.v.map(function(v,i){var dep=data.d[i];var p=v-dep;return dep>0?p/dep*100:NaN});
  return{p:pnl,ret:ret,dep:data.d,l:data.l}
}
async function drawPlTrend(period){
  ...
  canvas.width=W*dpr;canvas.height=H*dpr;canvas.style.width=W+"px";canvas.style.height=H+"px";
  canvas.getContext("2d").scale(dpr,dpr);
  var validRet=data.ret.filter(function(v){return!isNaN(v)});
  var mn=validRet.length?Math.min.apply(null,validRet):0,mx=validRet.length?Math.max.apply(null,validRet):0;
  if(mn>0)mn=0;if(mx<0)mx=0; // always include the zero line for a Return chart
  var rng=mx-mn||1;
  var padX=6,padY=10;
...
function drawPlTrendFrame(hoverIdx){
  ...
  var ctx=canvas.getContext("2d");
  var W=s.W,H=s.H,padX=s.padX,padY=s.padY,mn=s.mn,mx=s.mx,rng=s.rng,n=s.n;
  var series=s.data.ret,pnl=s.data.p;
  ctx.clearRect(0,0,W,H);
  function px(i){return padX+i/(n-1)*(W-padX*2)}
  function py(v){return H-padY-(v-mn)/rng*(H-padY*2)}
  try{
    ...
    for(var i=1;i<n;i++)ctx.lineTo(px(i),py(series[i]));
    ctx.lineTo(px(n-1),H-padY);ctx.lineTo(px(0),H-padY);ctx.closePath();
    ctx.fillStyle=grad;ctx.fill();
    ctx.beginPath();ctx.strokeStyle="#1565C0";ctx.lineWidth=2;ctx.lineJoin="round";ctx.lineCap="round";
  ...
  try{
    ctx.beginPath();ctx.arc(px(n-1),py(series[n-1]),3.5,0,Math.PI*2);ctx.fillStyle="#fff";ctx.fill();ctx.strokeStyle="#1565C0";ctx.lineWidth=2;ctx.stroke()
  }catch(e){console.error("[P/L Trends] endpoint dot draw failed:",e&&e.message)}
  try{
    ctx.font="700 10px DM Sans,sans-serif";ctx.textAlign="left";
    [mx,(mn+mx)/2,mn].forEach(function(v,gi){
      var yy=padY+gi*((H-padY*2)/2);
      var txt=(typeof portfolioVisible==="undefined"||portfolioVisible)?fmtPctAxisLabel(v):"••••";
      var tw=ctx.measureText(txt).width;
      ctx.fillStyle="rgba(255,255,255,.9)";ctx.fillRect(padX-2,yy-9,tw+8,14);
      ctx.fillStyle="#334155";ctx.fillText(txt,padX+2,yy+2)
    })
  ...
  try{
    if(hoverIdx!=null){
      ...
      ctx.beginPath();ctx.moveTo(px(idx),padY);ctx.lineTo(px(idx),H-padY);ctx.stroke();
      ctx.restore();
      ctx.beginPath();ctx.arc(px(idx),py(series[idx]),4,0,Math.PI*2);ctx.fillStyle="#1565C0";ctx.fill()
    }
  ...
  try{
    ...
    if(hDate&&hRows){
      ...
      hDate.textContent=dt.toLocaleDateString("en-MY",{day:"numeric",month:"short",year:"numeric"});
      var visible=typeof portfolioVisible==="undefined"||portfolioVisible;
      var val=pnl[idx],retPct=series[idx];
      var pnlCls=val>=0?"pnl-pos":"pnl-neg";
      var pnlStr=visible?(val>=0?"+":"-")+"RM "+fmtMoney(Math.abs(val)):"••••••";
      var retStr=!visible?"••••••":isNaN(retPct)?"—":(retPct>=0?"+":"")+retPct.toFixed(2)+"%";
      var tt=typeof t==="function"?t:function(k){return k};
      hRows.innerHTML=adHoverRow(tt("assetdetails.plValue"),pnlStr,pnlCls)+adHoverRow(tt("assetdetails.returnPct"),retStr,isNaN(retPct)?"":pnlCls)
    }
